...
WTVAL<-c(0,1.501,3,4.501)[woowoo]
wtsigma<-(-3.0)
outdir<-paste(basedir,'FTD_ROI_Long',studytype,round(WTVAL),sep='')
dir.create(outdir, showWarnings = FALSE)
WTVAL<-as.numeric(WTVAL)
if ( studytype == 'MD' ) {
  myRoiDescriptor<-data.frame(
    label_number=as.numeric(1:1000),
    label_name=paste("MDEanat",1:1000,sep='')
    )
}
##############################
##############################
...
if ( ! exists("myroidf") )
  ...
  myroidfArea<-myroidf
  colnames(myroidf)<-myRoiDescriptor$label_name
  colnames(myroidfThickness)<-myRoiDescriptor$label_name
  ct<-1
  for ( x in demog$expectedImagePrefix[haveSubject]  )
    {
    if ( studytype == 'MD' ) {
...
for ( jj in 1:ncol(myroidf) )
  ...
  ttdp<-as.numeric(subdemog$LikelyTDP)
#  ttdp<-as.numeric(subdemog$DefiniteTDP)
  ttdp<-ttdp+1
  ttdp[ mydx == "ELD CTRL" ]<-0 # these are controls
  ttdp<-as.factor(ttdp)
# do this in order to make time more interpretable wrt quadratic
  timevarb<-as.numeric(subdemog$MonthsDuration)/12.0
  timevarb[ timevarb>6 ]<-NA
  timevar<-timevarb-mean(timevarb,na.rm=T)
  ...
  if ( verbose == 1 ) {
    print(myRoiDescriptor$label_name[jj])
    print(smdl)  
...
#    plot( effect("poi_dTime*poi_DX",mdl))
  ...
  labnum<-as.numeric( myRoiDescriptor$label_number[jj] )
  if ( ncol(smdl) > 3 )
  {
  fxf<-fixef(mdl)
  if ( studytype != "MD")
